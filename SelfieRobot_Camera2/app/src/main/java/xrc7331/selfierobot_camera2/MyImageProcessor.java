 ...
 */
public class MyImageProcessor implements Observable<MyImageProcessor> {
    private static final int INPUT_PERMITS = 1;
    private static final Size BLUR_KERNEL = new Size(21, 21);

    private static MyImageProcessor INSTANCE = new MyImageProcessor();

    public static MyImageProcessor getInstance() {
        return INSTANCE;
    }

    private final Size MORPHOLOGY_KERNEL_SIZE = new Size(9, 9);
    ...
    private List<Observer<MyImageProcessor>> listeners = new LinkedList<>();
...
//        Imgproc.morphologyEx(binaryImage, binaryImage, Imgproc.MORPH_CLOSE, morphologyKernel);
    ...
    @Override
    public void addObserver(Observer<MyImageProcessor> observer) {
        listeners.add(observer);
    }

    @Override
    public void notifyObservers() {
        for (Observer<MyImageProcessor> listener : listeners)
